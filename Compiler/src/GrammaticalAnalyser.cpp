...
void GrammaticalAnalyser::analyseFuncDef() {
	...
	needReturn = function.getReturnType() == "int" || function.getReturnType() == "char";
...
bool GrammaticalAnalyser::analyseStmt() {
	...
	if (word.typeEquals("IDENFR")) {
		...
		whileLabels.emplace_back(std::unordered_map<std::string, std::string>{
		...
		codes.emplace_back(PCode(CodeType::LABEL,
			whileLabels.back()["while"], ValueType::STRING));
		getWord();//while
		forFlag++;
		getWord();//(
		analyseCond("WHILETK");
		checkParent(); //)
		codes.emplace_back(PCode(CodeType::JZ,
			whileLabels.back()["while_end"], ValueType::STRING));
		codes.emplace_back(PCode(CodeType::LABEL,
			whileLabels.back()["while_block"], ValueType::STRING));
		analyseStmt();
		forFlag--;
		codes.emplace_back(PCode(CodeType::JMP,
			whileLabels.back()["while"], ValueType::STRING));
...
void GrammaticalAnalyser::analyseConstInitVal() {
	...
	if (word.typeEquals("LBRACE")) {
		...
		for (int i = 1; i < length - 1; i++) {
			Word w("CHRCON", std::string(1, s[i]), word.getLineNum());
...
int GrammaticalAnalyser::analysePrimaryExp(std::vector<Word>& exp) {
	...
	if (word.typeEquals("LPARENT")) {
		...
		if (exp[exp.size() - 1].getContent() == ")")
		{
			...
			std::vector<Word> subVec(exp.begin() + 1, exp.end());
			analyseExp(subVec);
			error("j");
			//grammar.push_back(exp[exp.size() - 1].toString());
		}
	...
	else if (word.typeEquals("CHRCON")) {
...
Exps GrammaticalAnalyser::divideExp(std::vector<Word>& exp, std::vector<std::string>& symbol) {
	...
	for (int i = 0; i < exp.size(); i++) {
		...
		unaryFlag = word.typeEquals("IDENFR") || word.typeEquals("RPARENT") || word.typeEquals("INTCON") || word.typeEquals("CHRCON") || word.typeEquals("RBRACK");
	}
	exps.push_back(exp1);
...
void GrammaticalAnalyser::error(std::string type) {
	Errors::getErrors().push_back(Error(curWord.getLineNum(), type));
}
...
void GrammaticalAnalyser::printErrors() {
	std::ofstream errorWriter("error.txt");
	std::vector<Error> errors = Errors::getErrors();
